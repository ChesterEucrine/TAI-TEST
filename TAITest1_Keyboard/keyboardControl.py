...
# Key combo
def combo(keylist, doDelay=False):
    l = len(keylist)
    if l == 1:
        user32.keybd_event(keylist[0], 0, 0, 0)
        sleep(delay)
        user32.keybd_event(keylist[0], 0, 2, 0)
    else:
        i = 0
        while i < l:
            user32.keybd_event(keylist[i], 0, 0, 0)
            i += 1
        sleep(delay)
        while i >= 0:
            i -= 1
            user32.keybd_event(keylist[i], 0, 2, 0)
    if doDelay:
        sleep(delay*2)

#def sequence(listOfKeyList):
